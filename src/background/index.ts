import type { MsgFromContent, MsgFromPanel, MsgToContent, MsgToPanel, PageAnalysis, RedirectHop, TrackerHit } from "@shared/messages";

const analyses = new Map<number, PageAnalysis>();
const redirectChains = new Map<number, RedirectHop[]>();

// Simple in-memory cache for WHOIS and certificate lookups (24h TTL)
type CacheEntry<T> = { data: T; ts: number };
const CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const whoisCache = new Map<string, CacheEntry<{ domainAgeDays?: number }>>();
const certCache = new Map<string, CacheEntry<{ issuer?: string; notAfter?: string; issuedDaysAgo?: number }>>();
...
// --- Risk helpers and UI feedback ---
type RiskLevel = 'secure' | 'caution' | 'danger';
type SiteVerdict = import('@shared/messages').SiteVerdict;
type LogEntry = import('@shared/messages').LogEntry;

function classifyHostname(host: string): { spoofedLike?: string } {
...
function makeVerdict(analysis: PageAnalysis): SiteVerdict {
  const logs: LogEntry[] = [];
  ...
  if (typeof age === 'number') {
    if (age < 7) score -= 40; // very new domain
    else if (age < 30) score -= 20; // new domain
  }
  ...
  let risk: RiskLevel;
  if (score >= 90) risk = 'secure';
  else if (score >= 60) risk = 'caution';
  else risk = 'danger';

  const badgeText = risk === 'danger' ? '!' : (analysis.threats.trackers.length > 0 ? String(Math.min(analysis.threats.trackers.length, 99)) : '');

  return { risk, badgeText, logs, ...spoof, score } as SiteVerdict;
}

function setBadgeByRisk(tabId: number, verdict: SiteVerdict) {
...
// Messaging router
chrome.runtime.onMessage.addListener((msg: MsgFromContent | MsgFromPanel, sender, sendResponse) => {
  const tabId = sender.tab?.id ?? ("tabId" in msg ? (msg as any).tabId : undefined);
  ...
  if (msg.type === "CONTENT/REQUEST_ANALYSIS" && tabId != null) {
    ...
    if (!analyses.has(tabId)) {
      analyses.set(tabId, {
        ...
        summaries: {},
        verdict: makeVerdict({ tabId, url: msg.url, trust: { isHttps: msg.url.startsWith("https://"), hostname: new URL(msg.url).hostname }, consent: { findings: [] }, threats: { trackers: [], fingerprintingSignals: [] }, links: { redirectChain: redirectChains.get(tabId) ?? [] }, summaries: {} } as PageAnalysis)
      });
    }
  ...
  if (msg.type === "PANEL/GET_ANALYSIS") {
    ...
    if (targetTabId >= 0) {
      ...
      const out: MsgToPanel = analysis ? { type: "BG/ANALYSIS", analysis } : { type: "BG/STATUS", text: "No analysis yet. Interact with the page to start scanning." };
      sendResponse(out);
    }
  }
  ...
  if (msg.type === "PANEL/SUMMARIZE_PAGE" && tabId != null) {
    ...
    chrome.tabs.sendMessage(tabId, { type: "BG/REQUEST_PAGE_TEXT" as const });
  }

  if (msg.type === "PANEL/SUMMARIZE_SELECTION" && tabId != null) {
    chrome.tabs.sendMessage(tabId, { type: "BG/REQUEST_SELECTION_TEXT" as const });
  }
  ...
  if (msg.type === 'PANEL/LANG_LOG' && tabId != null) {
    const a = analyses.get(tabId);
    if (a) {
      const entry: LogEntry = { ts: Date.now(), level: 'info', text: msg.text, tag: 'lang' } as any;
      const verdict = (a.verdict ??= { risk: 'secure', logs: [] } as any);
...
chrome.runtime.onMessage.addListener((msg: any, sender, sendResponse) => {
  if (msg.type === "CONTENT/LINK_HOVER") {
    previewRedirect(msg.href).then((result) => {
      const out: MsgToContent = { type: "BG/HOVER_PREVIEW", finalUrl: result.finalUrl, chain: result.chain, hoverVerdict: result.hoverVerdict as any, reason: result.reason } as any;
      sendResponse(out);
    });
    return true;
  }
...
async function previewRedirect(href: string): Promise<{ finalUrl?: string; chain: RedirectHop[]; hoverVerdict?: 'safe'|'suspicious'|'danger'; reason?: string }> {
...
async function maybeEnrichTrust(tabId: number, url: string): Promise<void> {
  try {
    const a = analyses.get(tabId);
    if (!a) return;
    ...
    const whoisEndpoint: string = cfg?.whoisEndpoint || 'builtin:rdap';
    const certEndpoint: string = cfg?.certEndpoint || 'builtin:crtsh';
    const useMockData: boolean = cfg?.useMockData || false;
    let changed = false;
          ...
          if (whoisEndpoint === 'builtin:rdap') {
            ...
            if (res.ok) {
              ...
              const created = events.find(e => (e.eventAction || '').includes('registration') || (e.eventAction || '').includes('creation'))?.eventDate
                || data?.events?.find?.((e: any) => (e.eventAction || '').includes('registration'))?.eventDate;
              if (created) {
          ...
          if (certEndpoint === 'builtin:crtsh') {
            ...
            if (res.ok) {
              ...
              if (Array.isArray(arr) && arr.length > 0) {
                ...
                if (notAfter) a.trust.certExpiry = notAfter;
                if (issuedDaysAgo !== undefined) a.trust.certIssuedDaysAgo = issuedDaysAgo;
            ...
            if (res.ok) {
              ...
              if (issuer) a.trust.certIssuer = issuer;
              if (notAfter) a.trust.certExpiry = notAfter;
              if (issuedDaysAgo !== undefined) a.trust.certIssuedDaysAgo = issuedDaysAgo;
              cacheSet(certCache, host, { issuer, notAfter, issuedDaysAgo });
              changed = true;
    ...
    if (changed) {
      ...
      try {
        chrome.runtime.sendMessage({ type: 'BG/ANALYSIS', analysis: a } as any);
      } catch {}
    } else {
...
async function classifyWithWhois(host: string): Promise<{ verdict: 'safe'|'suspicious'|'danger'; reason: string }> {
  const spoof = classifyHostname(host);
  if (spoof.spoofedLike) return { verdict: 'danger', reason: `Possible spoof of ${spoof.spoofedLike}` };
  try {
    const cfg = await chrome.storage?.sync?.get?.(['whoisEndpoint']);
    const whoisEndpoint: string = cfg?.whoisEndpoint || 'builtin:rdap';
    let age: number | undefined = cacheGet(whoisCache, host)?.domainAgeDays;
    if (age === undefined) {
      if (whoisEndpoint === 'builtin:rdap') {
        const res = await fetch(`https://rdap.org/domain/${encodeURIComponent(host)}`);
        if (res.ok) {
    ...
    if (typeof age === 'number' && age < 30) return { verdict: 'suspicious', reason: 'Newly registered domain (<30 days)' };
  } catch {}
