...
export async function summarizeText(text: string): Promise<string | null> {
  try {
    ...
    const Summ = (globalThis as any).ai?.summarizer ?? (globalThis as any).Summarizer; // support both namespaced and global
    ...
    return `Summarizer error: ${(e as Error).message}`;
  }
}

// Language detection helpers
export async function detectLanguage(text: string): Promise<{ code?: string; probability?: number; note?: string }> {
  try {
    ...
    if (!avail || avail === 'no') return { note: 'Language detector model not available yet.' };
    // @ts-ignore
    const detector = await LD?.create?.();
    if (!detector?.detect) return { note: 'Language detector unavailable.' };
    ...
    const toCandidate = (x: any) => {
      if (!x) return { code: undefined, prob: undefined };
    ...
    return { note: `Language detection error: ${(e as Error).message}` };
  }
}

export async function translateToEnglish(_text: string): Promise<{ translated?: string; note?: string }> {
