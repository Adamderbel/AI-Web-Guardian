import type { ConsentFinding, MsgFromContent, RedirectHop, TrackerHit } from "@shared/messages";
import trackers from "@shared/trackers.json";

// Simple state
const findings: ConsentFinding[] = [];
const trackerHits: TrackerHit[] = [];
const fingerprintSignals: string[] = [];

function send(msg: MsgFromContent) {
...
function scanConsent() {
  ...
  // Hidden opt-outs
  const possibleOptOut = Array.from(document.querySelectorAll<HTMLElement>('a,button,input'))
    .filter((el) => /opt[- ]?out|reject|decline/i.test(el.innerText || (el as HTMLInputElement).value || ""));
  for (const el of possibleOptOut) {
    const cs = getComputedStyle(el);
    if (cs.display === "none" || cs.visibility === "hidden" || parseFloat(cs.opacity) < 0.1) {
      findings.push({ kind: "hidden-optout", text: el.innerText || (el as HTMLInputElement).value, selector: cssPath(el) });
    }
  }
...
function scanTrackers() {
  ...
  for (const h of hosts) {
    const hit = (trackers as any as { domain: string; type: TrackerHit["type"] }[]).find(t => h.endsWith(t.domain));
    if (hit) {
  ...
  if ((window as any).FingerprintJS || srcs.some(s => /fingerprint/i.test(s))) {
    fingerprintSignals.push("fingerprintjs detected");
  }
  const canvas = (HTMLCanvasElement as any).prototype?.toDataURL;
  if (canvas) {
    // heuristic marker
    fingerprintSignals.push("canvas API available (heuristic)");
  }

  if (trackerHits.length) send({ type: "CONTENT/TRACKERS", tabId: getTabId(), trackers: trackerHits });
  if (fingerprintSignals.length) send({ type: "CONTENT/FINGERPRINTING", tabId: getTabId(), signals: fingerprintSignals });
}
...
function setupLinkHoverPreview() {
  ...
  document.addEventListener('mouseover', (e) => {
    ...
    chrome.runtime.sendMessage({ type: 'CONTENT/LINK_HOVER', tabId: getTabId(), href: a.href }, (resp: any) => {
      if (!resp) return;
      const finalUrl = resp.finalUrl || a.href;
      const verdict = resp.hoverVerdict as ('safe'|'suspicious'|'danger'|undefined);
      const reason = resp.reason || '';
      let prefix = '↪︎';
      let label = '';
      let bg = '#111';
      let color = '#fff';
      if (verdict === 'danger') { label = '⚠️ Dangerous'; bg = '#b71c1c'; }
      else if (verdict === 'suspicious') { label = '⚠️ Suspicious'; bg = '#f57f17'; color = '#000'; }
      else { label = '✅ Safe'; bg = '#1b5e20'; }
